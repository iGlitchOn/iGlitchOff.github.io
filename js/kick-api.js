...
const KickAPI = (() => {
  ...
  async function getChannelInfo(username) {
    try {
      ...
      const token = await getToken();
      if (token) {
        const data = await apiFetch('/channels', { broadcaster_user_login: username });
        if (data?.data?.[0]) return normalizeChannel(data.data[0]);
      }
    } catch(e) {}
    // Fallback to public v1
    try {
      const data = await publicFetch(`/channels/${username}`);
  ...
  async function getVideos(username, page = 1) {
    try {
      const token = await getToken();
      if (token) {
        const data = await apiFetch('/videos', { broadcaster_user_login: username, page, per_page: 20 });
        return (data?.data || []).map(normalizeVideo);
      }
    } catch(e) {}
    // Fallback
    try {
      const data = await publicFetch(`/channels/${username}/videos?page=${page}&limit=20`);
